...
import { faCartShopping, faCircleCheck, faXmark } from "@fortawesome/free-solid-svg-icons"
            ...
            {cartShop.length > 0 &&
                <div className="content bg-white shadow-md p-2 rounded-md w-full max-w-[400px] mx-2">
                    ...
                    <div className="flex flex-col items-center gap-1.5 my-2.5">
                        <img src={"/webshop-2/" + cartShop[cartShop.length - 1].lista_img[0] + ".jpg"} alt="" className="max-w-[180px] rounded-sm" />
                        <h3 className="text-xs flex items-center gap-1">{cartShop[cartShop.length - 1].nome}</h3>
                    </div>

                    <button className="text-[.8rem] font-semibold p-3 bg-red-800 text-white hover:bg-red-950 w-full" onClick={() => {
                        closeModal("new-item-modal")
                        openModal("cart-modal")
                    }}>Ver carrinho <FontAwesomeIcon icon={faCartShopping} /></button>
                </div>
            }
        </div>
    )
